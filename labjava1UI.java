...
public class labjava1UI extends javax.swing.JFrame {
    ...
    int activeRow;
    int activeRow1;
    public class CulcThread1 extends Thread {   
        @Override
        public void run(){
            DefaultTableModel dt = (DefaultTableModel) jTable1.getModel();
            for(activeRow = row; activeRow < middle ; activeRow++){
                if(row != -1){
                    dt.setValueAt(integralList.get(activeRow).integralCalculate(), activeRow, 3);
     ...
     public class CulcThread2 extends Thread {   
        @Override
        public void run(){
            DefaultTableModel dt = (DefaultTableModel) jTable1.getModel();
            for(activeRow1 = middle; activeRow1 < row + rowCount ; activeRow1++){
                if(row != -1){
                    dt.setValueAt(integralList.get(activeRow1).integralCalculate(), activeRow1, 3);
                    System.out.println(Thread.currentThread().getName());
                }
                try {
                   Thread.sleep(1000);
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        rowCount = jTable1.getSelectedRowCount();
        CulcThread1 Thread1 = new CulcThread1();
        CulcThread2 Thread2 = new CulcThread2();
        if ( rowCount % 2 == 0){
                middle = (row + rowCount)/2;
            }else {middle = (row + rowCount)/2 + 1;}
        Thread1.start();
        Thread2.start();
